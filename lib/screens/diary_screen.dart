...
/// БАГ #7: ключ кэша изображения карточки записи. URL фото ФИКСИРОВАН
/// (не меняется при перезаписи), поэтому по нему нельзя отличить новое фото
/// от старого. Версию содержимого несёт путь файла (см. `_syncPhoto` в engine —
/// в имя файла вшита миллисекунда updated_at), поэтому приоритет — у photoPath:
/// как только на устройство приходит новая версия, путь меняется, ключ меняется,
/// и Image.file пересоздаётся, показывая актуальный кадр (а не устаревший из кэша
/// Flutter). Падение на photoUrl — только для записей без скачанного файла.
String photoCacheKey(DiaryEntry e) =>
    'photo-${e.photoPath ?? e.photoUrl ?? 'none'}';
